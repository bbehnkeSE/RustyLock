...
fn main() {
    ...
    if args.len() == 1 {  
        ...
        match opt.control {
            Control::Encrypt => {
                let enc_result = locker::encrypt(&key, &pth);
                match enc_result {
                    Ok(_) => (),
            ...
            Control::Decrypt => {
                let dec_result = locker::decrypt(&key, &pth);
                match dec_result {
                    Ok(_) => (),
                    Err(e) => fs::write("./rusty_error.log", e.to_string())
                        .expect("Failed to write to error log")
                            ...
                            match self.opts.control {
                                Control::Encrypt => {
                                    for path in self.opts.paths.clone() {
                                        let enc_result = locker::encrypt(&gen_key(self.password.as_bytes()), &path);
                                        match enc_result {
                                            Ok(_) => (),
                                            Err(e) => fs::write("./rusty_error.log", e.to_string())
                                                .expect("Failed to write to error log")
                                ...
                                Control::Decrypt => {
                                    for path in self.opts.paths.clone() {
                                        let dec_result = locker::decrypt(&gen_key(self.password.as_bytes()), &path);
                                        match dec_result {
                                            Ok(_) => (),
                                            Err(e) => fs::write("./rusty_error.log", e.to_string())
                                                .expect("Failed to write to error log")
